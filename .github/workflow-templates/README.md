...
**Что запущено — в переменных `REF_*`.** В пользовательских командах доступны `REF_TYPE`
(`branch` / `tag` / `pr`), `REF_NAME` (ref как есть), `REF_NAME_NORM` (готовый идентификатор:
без слешей и без `v`), `REF_BRANCH` и `REF_COMMIT`. Собирать их из `github.ref_name` не нужно
и не надо:
...
Имена защищены от подмены: переменная проекта с именем `REF_NAME` до команд не доедет.
Таблица значений — в [документации ci-cd](../../docs/.github/workflows/ci-cd.yml.md#переменные-ref-что-именно-запущено).
