...
- `PAT_TOKEN` виден внутри `BUILD_COMMAND` и `CI_COMMAND` как `$GH_TOKEN`. Через него настраивается
  доступ к приватным зависимостям — например `git config --global url."…".insteadOf` для приватных
  зеркал Go-модулей. Сам токен в переменную не пишется: в `vars` попадают только правила, значение
  подставляет шаг, и GitHub маскирует его в логах.
