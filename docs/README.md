...
- В пользовательских командах (`BUILD_COMMAND`, `CI_COMMAND`, `BEFORE_/AFTER_DEPLOY_COMMAND`, сборка
  образа) доступны две переменные: `$GH_TOKEN` — встроенный `GITHUB_TOKEN`, права только на текущий
  репозиторий; `$PAT_TOKEN` — секрет `PAT_TOKEN`, нужен для приватных зависимостей из **чужих**
  репозиториев (`git config --global url."…".insteadOf` для приватных зеркал Go-модулей). Сам токен
  в переменную не пишется: в `vars` попадают только правила, значение подставляет шаг, и GitHub
  маскирует его в логах. Подробности — в [документации ci-cd](.github/workflows/ci-cd.yml.md).
